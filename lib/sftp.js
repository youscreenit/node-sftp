...
(function() {
  ...
  this.connect = function(cbconnect) {
    ...
    this.callbacks["connect_err"] = function(err) {
      console.log('called connect err');
      _self.state = Sftp.STATE_DISCONNECTED;
        ...
        var waitThatFileWasWritten = function(name, innerDone) {
          _self.stat(name, function(err) {
            if (err) {
              ...
              tries += 1;
              return waitThatFileWasWritten(name, innerDone);
            }
  ...
  function parseReply(data) {
    ...
    if (this.state & Sftp.STATE_CONNECTING) {
      if (data.indexOf('Operation timed out') !== -1) {
        this.callbacks["connect_err"](new Error('Operation timed out'));
        delete this.callbacks["connect_err"];
        delete this.callbacks["connect"];
        return;
      }

      if (data.indexOf('Connection closed') !== -1) {
        this.callbacks["connect_err"](new Error('Connection closed'));
        delete this.callbacks["connect_err"];
        delete this.callbacks["connect"];
        return;
      }
      if (data.indexOf('Connection timed out') !== -1) {
        this.callbacks["connect_err"](new Error('Connection timed out'));
        delete this.callbacks["connect_err"];
        delete this.callbacks["connect"];
        return;
      }
      ...
      if (data.indexOf('Connection reset by peer') !== -1) {
        this.callbacks["connect_err"](new Error('Connection reset by peer'));
        delete this.callbacks["connect_err"];
        delete this.callbacks["connect"];
        return;
      }

      if (data.indexOf('Permission denied (publickey).') !== -1) {
        this.callbacks["connect_err"](new Error('permission denied'));
        delete this.callbacks["connect_err"];
        delete this.callbacks["connect"];
        return;
      }
